/**Given an integer n, find the closest integer (not including itself), which is a palindrome.
 ...
 To understand this method, let's start with a simple illustration. Assume that the number given to us is "abcxy".
 One way to convert this number into a palindrome is to replicate one half of the string to the other half.
 If we try replicating the second half to the first half, the new palindrome obtained will be "yxcxy" which lies at
 an absolute of ∣10000(a−y)+1000(b−x)| from the original number.
 But, if we replicate the first half to the second half of the string, we obtain "abcba", which lies at an absolute
 ...
 ut of these three palindromes, we can choose the one with a minimum difference from nnn. Further, in case of a tie,
 we need to return the smallest palindrome obtained. For resolving this tie's conflict,
 we can observe that a tie is possible only if one number is larger than nnn and another is lesser than n.
 Further, we know that the number bbb is obtained by decreasing nnn. Thus, in case of conflict between bbb and
 ...
 Time complexity : O(n). Scanning, insertion, deletion,, mirroring takes O(n), where n is the length of the string.
