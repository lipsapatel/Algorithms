 ...
 * Count the number of prime numbers less than a non-negative number, n.
 ...
 But don't let that name scare you, I promise that the concept is
 surprisingly simple.
 ...
 We know all multiples of 2 must not be primes,
 so we mark them off as non-primes.
 Then we look at the next number, 3. Similarly, all multiples of 3
 ...
 4 is not a prime because it is divisible by 2, which means all multiples
 of 4 must also be divisible by 2
 and were already marked off. So we can skip 4 immediately and go to the next
 number, 5.
 Now, all multiples of 5 such as 5 × 2 = 10, 5 × 3 = 15, 5 × 4 = 20,
 5 × 5 = 25, ... can be marked off.
 There is a slight optimization here,
 ...
 because 5 × 2 = 10 was already marked off by
 ...
 of p starting at p2, then in
 increments of p: p2 + p, p2 + 2p, ... Now what should be the
 ...
 Yes, the terminating loop condition can be p < √n,
 as all non-primes ≥ √n must have already been marked off.
 When the loop terminates, all the numbers in the table that are
 non-marked are prime.

 The Sieve of Eratosthenes uses an extra O(n) memory and its
 runtime complexity is O(n log log n).
 For the more mathematically inclined readers, you can read more
...
public class CountPrimes {
    ...
    private static int countPrimesSieveEratosthenes(int n) {
        ...
        for (int i = 2; i * i <= n; i++) { //O(n)
            ...
            for (int j = i * i; j < n; j += i) { //O(loglogn) n/2, n/3, n/5...
                isPrime[j] = false;
            }
        }
