 ...
 *
 * Recursive Approach
 * 1) Start at index 0 and cut string at different point.
 * 2) Check if the string is palindrome.
 * 3) If it is palindrome, then repeat the process for the rest of the string.
 * 4) Recursive function (String input, int start, List<String> list)
 * 5) Base case: If start == input.length() then print the list
 * 6) Recursive case: for (i = start; i < input.length), check if the string (start, i) is palindrome
 * 7) If it is palindrome then add to list and make recursive call (input, i + 1, list)
 * 8) Remove from list for backtrack or revert changes.
 *
 * Time Complexity: O(n^n * n)
 * Space Complexity: O(n)
 *
 * resources/IK.Recursion.Class.PalindromicDecompositionOfString.png
 *
 ...
 * We have provided two solutions:
 ...
 In solution using dynamic programming we have pre-calculated is_palindrome array, but in solution using recursion we have not.
 But you should do it in recursive solution also. (To make code readable and easy to understand, we have not pre-calculated in recursive solution.)
...
public class PalindromicDecompositionOfString {
    ...
    //*****************************************************************************************************************
    //Generate pre calculate palindrome array
