 ...
 * <= > ? pivot
 *
 * Lomuto performs relatively poorly when there is high percentage of duplicate values where
 * as Hoare performs just fine in that situation.
 *
 * Lomotu – pi = 5 skew every time swap O(n2) while hoare performs in O(nlogn)
 ...
 Hoare's Algorithm performance is degraded to O(n^2) if the array is already sorted.
 ...
 fewer swaps on average, and it creates efficient partitions even when all values are equal.
