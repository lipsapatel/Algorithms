...
public class FindSubset {
    ...
    private static String[] generate_all_subsets(String s) {
        ...
        char[] subsets = new char[s.length()];

        generateAllSubsets(s.toCharArray(), 0, result, subsets, 0);

        return result.toArray(new String[0]);
    }

    private static void generateAllSubsets(char[] s, int i, List<String> result, char[] subsets, int j) {

        //Base Case
        if (i == s.length) {
            result.add(new String(subsets, 0, j));
        } else { //Recursive Case

            //Not include
            generateAllSubsets(s, i + 1, result, subsets, j);

            //Include
            subsets[j] = s[i];
            generateAllSubsets(s, i + 1, result, subsets, j + 1);
        }
    }
